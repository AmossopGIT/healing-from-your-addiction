...
1. `new`: received but not reviewed
2. `triage_review`: intake reviewed and safety checked
3. `outreach_started`: first response sent
4. `care_pathway_defined`: next-step plan agreed (assessment/invite/hybrid support)
5. `qualified`: suitable and willing to continue
6. `enrolled`: invited/enrolled to client workflow
7. `closed`: no further action for now
